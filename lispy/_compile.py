...
def compile_bytecode(exp):
    ...
    if isinstance(exp, list):
        ...
        if exp[0] == 'define':
            ...
            return compile_bytecode(['lambda', params, body]) \
                + [STORE_NAME(name)]
        if exp[0] == 'begin':
            return [inst for subexp in exp[1:]
                    for inst in compile_bytecode(subexp)]
        args = exp[1:]
        nargs = len(args)
        load_func = compile_bytecode(exp[0])
        compiled_args = list(
            chain.from_iterable(compile_bytecode(arg) for arg in args))
